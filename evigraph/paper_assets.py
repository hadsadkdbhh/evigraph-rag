...
from evigraph.process_trace import PROCESS_STEPS, ProcessTraceAnalyzer
...
FINQA_600_SUBMISSION_COMPONENT_CLOSURE_V48_RESULT_SPECS = (
    ResultSpec(
        "Oracle-doc",
...
RESULT_SPEC_PRESETS = {
    ...
    "finqa_600_submission_component_closure_v48": FINQA_600_SUBMISSION_COMPONENT_CLOSURE_V48_RESULT_SPECS,
...
class PaperAssetBuilder:
    ...
    def build(
        ...
        process_rows = self._process_rows(eval_path, specs)
        paths = {
            "latex": str(output_path / "finqa_results_tables.tex"),
            "main_latex": str(output_path / "finqa_main_tables.tex"),
            "markdown": str(output_path / "finqa_results_summary.md"),
        }
        Path(paths["latex"]).write_text(
            self.render_latex(result_rows, contribution_rows, failure_rows, diagnostic_rows, process_rows),
            encoding="utf-8",
        )
        Path(paths["main_latex"]).write_text(
            self.render_latex(
                result_rows,
                contribution_rows,
                failure_rows,
            ...
            for row in process_rows:
                lines.append(
                    " & ".join(
                        [
                            self._latex_escape(row["setting"]),
                            self._fmt(row["evidence_available"]),
                            self._fmt(row["period_hit"]),
                            self._fmt(row["row_hit"]),
                            self._fmt(row["operand_hit"]),
                            self._fmt(row["operation_hit"]),
                            self._fmt(row["citation_hit"]),
                            self._fmt(row["answer_supported"]),
                            self._fmt(row["exact_match"]),
                        ]
                    )
                    + " \\\\"
                )
    ...
    def _process_rows(self, eval_dir: Path, specs: tuple[ResultSpec, ...]) -> list[dict[str, Any]]:
        rows = []
        analyzer = ProcessTraceAnalyzer()
        for spec in specs:
            analysis = analyzer.analyze(self._spec_csv_path(eval_dir, spec), method=self._diagnostic_method(spec))
            row = {"setting": spec.label}
            for step in PROCESS_STEPS:
                row[step] = analysis["step_rates"].get(step, 0.0)
            rows.append(row)
        return rows
