...
- On the checked-in 100-example FinQA smoke subset, the current oracle-document setting reaches 57/100 numeric exact-match accuracy with transparent calculations for ratio, percent-of-total, percent-change, row-average, row/column lookup, year-range-average, ROI, prose average, cross-chunk ratio, and difference cases.
- Open BM25 reaches 50/100, deterministic open hybrid reaches 49/100, and source-rerank reaches 58/100 for full EviGraph on the same subset; these are diagnostic baselines, not final claims.
...
- Improve real-table numerical reasoning; current diagnostics point first to cross-chunk period-end table stitching, operand selection for ratio/percent-change calculations, and then ambiguous supported wrong-number cases.
